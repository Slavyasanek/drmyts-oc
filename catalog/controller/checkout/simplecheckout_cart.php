...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        // --- CUSTOM FREE DELIVERY PROGRESS BAR LOGIC ---
        // OpenCart 3 uses 'shipping_free_status', OpenCart 2 uses 'free_status'. We check both.
        $free_status = $this->config->get('shipping_free_status') ?: $this->config->get('free_status');

        if ($free_status) {
            $this->_templateData['show_free_delivery_bar'] = true;
            
            // Get the threshold from the admin panel
            $threshold = (float)($this->config->get('shipping_free_total') ?: $this->config->get('free_total'));
            $subtotal = $this->cart->getSubTotal();

            if ($subtotal < $threshold) {
                $this->_templateData['is_free_delivery'] = false;
                // Using Simple Checkout's built-in currency formatter
                $this->_templateData['free_delivery_left'] = $this->simplecheckout->formatCurrency($threshold - $subtotal); 
                $this->_templateData['free_delivery_percent'] = round(($subtotal / $threshold) * 100);
            } else {
                $this->_templateData['is_free_delivery'] = true;
                $this->_templateData['free_delivery_left'] = 0;
                $this->_templateData['free_delivery_percent'] = 100;
            }
        } else {
            // If disabled in admin, don't show the bar at all
            $this->_templateData['show_free_delivery_bar'] = false;
        }
        // -----------------------------------------------
