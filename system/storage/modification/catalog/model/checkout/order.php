...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
                ...
                if ($order_info && $order_info['customer_id']) {
                    ...
                    $query = $this->db->query("SELECT consult_coupon FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$order_info['customer_id'] . "'");
                    
                    if (!empty($query->row['consult_coupon'])) {
                        $saved_coupon = $query->row['consult_coupon'];
                        $order_total_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$order_id . "' AND code = 'coupon'");
                        
                        if ($order_total_query->num_rows) {
                            $this->model_extension_module_consultation->clearCustomerCoupon($order_info['customer_id']);
                        }
                    }
                }
