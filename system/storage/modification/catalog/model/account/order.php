...
class ModelAccountOrder extends Model {
	...
	public function getOrders($data = array()) {
                // Compatibility & Variable Initialization
                if (!is_array($data)) {
                    $start = $data;
                    $limit = func_num_args() > 1 ? func_get_arg(1) : 20;
		...
		if ($limit < 1) {
          ...
          // Building query dynamically to allow filters
          ...
          if (isset($data['filter_time'])) {
              if ($data['filter_time'] == 'month') {
                  $sql .= " AND MONTH(o.date_added) = MONTH(NOW()) AND YEAR(o.date_added) = YEAR(NOW())";
              } elseif ($data['filter_time'] == 'last_month') {
                  $sql .= " AND o.date_added >= DATE_SUB(NOW(), INTERVAL 1 MONTH)";
              } elseif ($data['filter_time'] == 'year') {
                  $sql .= " AND YEAR(o.date_added) = YEAR(NOW())";
              } elseif ($data['filter_time'] == 'last_year') {
                  $sql .= " AND YEAR(o.date_added) = YEAR(NOW()) - 1";
              }
          }
