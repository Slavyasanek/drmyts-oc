...
class ModelCatalogProduct extends Model {
    ...
    public function getMinMaxPrice($data = array()) {
        $sql = "SELECT 
                    MIN( IFNULL(ps.price, p.price) + 
                        IFNULL(
                            (
                                SELECT (SUM((IFNULL(ps.price, p.price) * tr.rate) / 100) + 2)  
                                FROM " . DB_PREFIX . "tax_rate tr 
                                LEFT JOIN " . DB_PREFIX . "tax_rule t_rule 
                                ON tr.tax_rate_id = t_rule.tax_rate_id 
                                WHERE t_rule.tax_class_id = p.tax_class_id 
                                AND tr.type = 'P'
                            ), 0
                        )
                    ) as min_price,
                    
                    MAX( IFNULL(ps.price, p.price) + 
                        IFNULL(
                            (
                                SELECT (SUM((IFNULL(ps.price, p.price) * tr.rate) / 100) + 2) 
                                FROM " . DB_PREFIX . "tax_rate tr 
                ...
                -- Standard Store & Visibility Joins
                LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)";

        // Join Category if filtered
        if (!empty($data['filter_category_id'])) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";
        }

        // Join Filters if filtered
        ...
        if (!empty($data['filter_category_id'])) {
            if (!empty($data['filter_sub_category'])) {
                $sql .= " AND p2c.category_id IN (
                    ...
                    WHERE path_id = '" . (int)$data['filter_category_id'] . "'
                )";
            } else {
        ...
        if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            if (!empty($data['filter_name'])) {
                ...
                foreach ($words as $word) {
                    $implode[] = "p.product_id IN (SELECT pd.product_id FROM " . DB_PREFIX . "product_description pd WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pd.name LIKE '%" . $this->db->escape($word) . "%')";
                }
            ...
            if (!empty($data['filter_tag'])) {
                $sql .= "p.product_id IN (SELECT pd.product_id FROM " . DB_PREFIX . "product_description pd WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pd.tag LIKE '%" . $this->db->escape($data['filter_tag']) . "%')";
            }

            $sql .= ")";
        }

        // Filter: OpenCart Standard Filters
        if (!empty($data['filter_filter'])) {
            $implode = array();
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
          ...
          $sql .= " LEFT JOIN " . DB_PREFIX . "product_special ps ON (p.product_id = ps.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ps.date_start <= NOW() AND (ps.date_end = '0000-00-00' OR ps.date_end >= NOW()))";
           }
      ...
      if (!empty($data['price_min']) && $data['price_min'] != '') {
      ...
      if (!empty($data['price_max']) && $data['price_max'] != '') {
...
'color_code'             => isset($product_option_value['color_code']) ? $product_option_value['color_code'] : '',
