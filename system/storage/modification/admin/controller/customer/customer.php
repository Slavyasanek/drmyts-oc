...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getForm() {
        ...
        // Додаємо логіку консультацій спеціально для форми редагування
        $this->load->language('sale/consultation');
        $this->load->model('sale/consultation');

        $data['customer_consultations'] = array();

        if (isset($this->request->get['customer_id'])) {
            $consult_results = $this->model_sale_consultation->getConsultationsByCustomerId($this->request->get['customer_id']);

            foreach ($consult_results as $result) {
                $data['customer_consultations'][] = array(
                    'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
                    'telegram'   => $result['telegram_id'],
                    'status'     => $result['status'] ? $this->language->get('text_processed') : $this->language->get('text_pending'),
                    'status_raw' => $result['status'],
                    'href'       => $result['status'] 
                        ? $this->url->link('sale/consultation/edit', 'user_token=' . $this->session->data['user_token'] . '&consultation_id=' . $result['consultation_id'], true)
                        : $this->url->link('sale/consultation/create', 'user_token=' . $this->session->data['user_token'] . '&consultation_id=' . $result['consultation_id'], true)
                );
            }
        }
        
        $data['tab_consultation'] = $this->language->get('heading_title');
