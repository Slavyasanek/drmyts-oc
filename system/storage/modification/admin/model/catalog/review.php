...
class ModelCatalogReview extends Model {
	...
	public function deleteReview($review_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "review WHERE review_id = '" . (int)$review_id . "'");
                ...
                $images = $this->db->query("SELECT image FROM " . DB_PREFIX . "review_image WHERE review_id = '" . (int)$review_id . "'");

            // 2. Loop through the results and delete the physical files from the server
            foreach ($images->rows as $img) {
                $file_path = DIR_IMAGE . $img['image'];
                
                // Check if the file actually exists before trying to delete it
                if (is_file($file_path)) {
                    unlink($file_path); // This deletes the actual file from the folder
                }
            }
